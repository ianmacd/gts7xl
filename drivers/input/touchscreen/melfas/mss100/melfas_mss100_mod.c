...
int mms_parse_devicetree(struct device *dev, struct mms_ts_info *info)
{
	...
	input_info(true, dev, "%s: fw_name %s int:%d irq:%d support_LPM:%d AOT:%d FOD:%d ED:%d\n",
		__func__, info->dtdata->fw_name, info->dtdata->gpio_intr, info->client->irq, 
		info->dtdata->support_lpm, info->dtdata->enable_settings_aot,
