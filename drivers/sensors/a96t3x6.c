...
#ifdef CONFIG_SENSORS_A96T3X6_2CH
	if (grip_thd_2ch < data->diff_2ch && !data->skip_event) {
...
static int a96t3x6_i2c_read_checksum(struct a96t3x6_data *data, u8 cmd)
{
	...
	SENSOR_INFO("Write Result (P:0x5A, F:0x0A) : %X\n", checksum[3]);
...
static int a96t3x6_fw_update(struct a96t3x6_data *data, u8 cmd)
{
	...
	while(retry2 > 0)
	{
		...
		while(retry > 0)
		{
			...
			if (ret < 0)
				SENSOR_ERR("a96t3x6_fw_mode_enter fail, retry : %d, %d\n",
	...
	while (retry--) {
		...
		if (cmd == BUILT_IN) {
			...
			if ((data->checksum_h != data->firm_data_ums[8]) ||
				...
				SENSOR_ERR("(SD card) checksum fail.(0x%x,0x%x),(0x%x,0x%x) retry:%d\n",
						data->checksum_h, data->checksum_l,
